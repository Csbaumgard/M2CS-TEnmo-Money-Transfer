...
> Note: there are two check constraints in the DDL that creates the `transfer` table. Be sure to take a look at `schema.sql` to understand these constraints.

## How to set up the database

Inside the `<project-root>/tenmo-server/database/` directory, you'll find an executable Bash script (`.sh` file) and several SQL scripts (`.sql` files). These can be used to build and rebuild a PostgreSQL database for the capstone project.

From a terminal session, execute the following commands:

```
cd <project-root>/tenmo-server/database/
./create.sh
```

This Bash script drops the existing database, if necessary, creates a new database named `tenmo`, and runs the various SQL scripts in the correct order. You don't need to modify the Bash script unless you want to change the database name.
...
Remember that when testing, you're using a copy of the real database. The schema is defined by `schema.sql`, but the data for the test database is defined within `/src/test/resources/test-data.sql`.
